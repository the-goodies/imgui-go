...
const (
	...
	// TableColumnFlagsDisabled overriding/master disable flag: hide column, won't show in context menu (unlike calling TableSetColumnEnabled() which manipulates the user accessible state)
	...
	// TableColumnFlagsNoHeaderLabel TableHeadersRow() will not submit label for this column. Convenient for some small columns. Name will still appear in context menu.
	TableColumnFlagsNoHeaderLabel = 1 << 12
...
// TableSetColumnEnabled change user accessible enabled/disabled state of a column. Set to false to hide the column.
// User can use the context menu to change this themselves (right-click in headers, or right-click in columns body with TableFlagsContextMenuInBody)
func TableSetColumnEnabled(columnN int, v bool) {
	C.iggTableSetColumnEnabled(C.int(columnN), castBool(v))
}
